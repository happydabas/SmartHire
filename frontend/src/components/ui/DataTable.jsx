  ...
  return (
    <div className={twMerge('w-full relative', className)}>
      ...
      <div className="block sm:hidden space-y-4">
        {!data || data.length === 0 ? (
          <div className="bg-white dark:bg-[#15161e] border border-slate-100 dark:border-slate-800 rounded-2xl p-8">
            {emptyState || (
              <p className="text-center text-sm font-semibold text-slate-400 dark:text-slate-500">No records available</p>
            )}
          </div>
        ) : (
          data.map((row, index) => {
            ...
            return (
              <div key={key} className="w-full">
                {renderMobileCard ? (
                  ...
                  <div className="bg-white dark:bg-[#15161e] border border-slate-100 dark:border-slate-800 rounded-2xl p-5 shadow-sm space-y-3.5">
                    {columns.map((col, colIdx) => {
                      if (col.header === 'Action' || col.header === 'Actions') {
                        return (
                          <div key={colIdx} className="pt-3.5 border-t border-slate-100 dark:border-slate-800 flex items-center justify-end gap-2.5">
                      ...
                      return (
                        <div key={colIdx} className="flex justify-between text-xs font-semibold">
                          <span className="text-slate-400 dark:text-slate-500 uppercase tracking-wider">{col.header}</span>
