...
export function LearningRecommendationsCard({ recommendations = [] }) {
    ...
    <div className="bg-white border border-slate-100 rounded-3xl p-6 shadow-sm dark:bg-slate-900 dark:border-slate-800 space-y-4 animate-fadeIn">
      <div>
        ...
        <p className="text-[10px] text-slate-505 font-semibold dark:text-slate-455 mt-0.5">
          Step-by-step training tips calculated by AI.
        </p>
      </div>

      {sortedRecs.length === 0 ? (
        <p className="text-xs text-slate-500 font-bold">No recommendations needed. Profile is highly matching!</p>
      ) : (
        <div className="space-y-3">
          {sortedRecs.map((item, idx) => (
            <div key={idx} className="flex gap-4 items-start p-3 bg-slate-50/50 border border-slate-100 rounded-2xl dark:bg-slate-950/10 dark:border-slate-800">
              ...
              <p className="text-xs text-slate-700 dark:text-slate-305 font-semibold leading-relaxed">
                {item.recommendation}
              </p>
            </div>
          ))}
